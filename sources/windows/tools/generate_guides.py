...
GUIDE_IDENTITY_LABELS = {
    "fr": "LUFScale pour Windows - version {version}",
    "en": "LUFScale for Windows - version {version}",
    "es": "LUFScale para Windows - versión {version}",
    "it": "LUFScale per Windows - versione {version}",
    "pt": "LUFScale para Windows - versão {version}",
    "ru": "LUFScale для Windows - версия {version}",
    "ja": "Windows版 LUFScale - バージョン {version}",
    "hi": "Windows के लिए LUFScale - संस्करण {version}",
    "zh": "LUFScale Windows 版 - 版本 {version}",
    "ko": "Windows용 LUFScale - 버전 {version}",
    "id": "LUFScale untuk Windows - versi {version}",
    "tr": "Windows için LUFScale - sürüm {version}",
}
...
def draw_simple_interface(
    ...
    c.setFillColor(field)
    c.setStrokeColor(border)
    c.roundRect(progress_x, progress_y, progress_width, 14, 3, fill=1, stroke=1)
    c.setFillColor(cyan)
    c.roundRect(
        progress_x + 1,
        progress_y + 1,
        (progress_width - 2) * 0.58,
        12,
        2,
        fill=1,
        stroke=0,
    ...
    c.setFillColor(colors.white)
    c.setFont(bold, 4.8)
    c.drawRightString(cpu_right_x, progress_y + 5, "CPU 48 %")
    c.setFillColor(BLUE)
    c.roundRect(
        start_button_x,
        progress_y - 1,
...
def page_quick_start(c, language, language_name, labels, regular, bold) -> None:
    top = draw_page_header(
        c,
        labels["guide"],
        GUIDE_IDENTITY_LABELS.get(
            language,
            GUIDE_IDENTITY_LABELS["en"],
...
def page_technical(c, language, language_name, labels, regular, bold) -> None:
    ...
    # The French guide defines the shared eight-card structure used by every
    # language. Keep all cards visible so translated guides cannot silently
    # lose the automatic-quality-control or parallel-processing sections.
    compact_card_rows = 4
    compact_cards_bottom = 236
    draw_two_column_cards(
        c, cards, 590, compact_cards_bottom,
        regular,
        bold,
        rows=compact_card_rows,
        body_max=DOCUMENT_BODY_SIZE,
    )
    draw_card(
        c,
...
def generate_guide(
    ...
    document.setSubject(f"LUFScale Windows visual guide - {language_name}")
