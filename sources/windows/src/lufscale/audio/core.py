...
from ..i18n.catalog_windows import EXTRA_CORE_TEXTS
...
def is_supported_audio_file(path: Path | str) -> bool:
    candidate = Path(path)
    # macOS stores Finder metadata and resource forks in AppleDouble sidecars
    # whose names begin with ``._``.  When such a folder is copied or
    # extracted on Windows, the sidecars keep the extension of the real audio
    # file (for example ``._song.mp3``) but contain no playable audio stream.
    # Reject them before recursive discovery so FFmpeg never receives them as
    # conversion jobs.  Other dot-prefixed audio names remain valid.
    if candidate.name.startswith("._"):
        return False
    return candidate.suffix.lower() in SUPPORTED_AUDIO_EXTENSIONS
...
def _destination_key(path: Path) -> str:
    """Compare relative destinations with Windows path semantics."""
    return str(path).replace("\\", "/").casefold()


def _unique_destination_path(relative: Path, used: set[str]) -> Path:
    """Return a unique relative path while preserving its real extension.
    ...
    format and rejected the second file.  The counter belongs in the stem so
    every destination remains a supported audio path.  The parent path stays
    unchanged, so genuine subfolders selected below a source root are retained.
    """
    path = Path(relative)
    candidate = path
    counter = 2
    while _destination_key(candidate) in used:
        candidate = path.with_name(f"{path.stem}__{counter}{path.suffix}")
        counter += 1
    used.add(_destination_key(candidate))
    return candidate
    ...
    for source_root in roots:
        if source_root.is_dir():
            for source in iter_audio_files(source_root):
                ...
                destination_relative = _unique_destination_path(
                    relative,
                    used_destinations,
                )
                jobs.append(
                    ConversionJob(source, output / destination_relative)
                )
        else:
            destination_relative = _unique_destination_path(
            ...
            jobs.append(
                ConversionJob(source_root, output / destination_relative)
            )
